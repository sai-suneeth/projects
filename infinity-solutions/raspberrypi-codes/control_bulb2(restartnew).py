...
log_file_path = "/home/kaboom-gaming/my_rfid_project/log_file.log"
# Custom logging format
log_format = '%(asctime)s - %(message)s'
logging.basicConfig(level=logging.INFO, format=log_format, handlers=[
    logging.FileHandler(log_file_path),
    logging.StreamHandler(sys.stdout)  # To log to both console and file
])

# Custom class to capture all console output (stdout and stderr)
class ConsoleLogger(object):
    def __init__(self):
        self.terminal_stdout = sys.stdout  # Save the default stdout
        self.terminal_stderr = sys.stderr  # Save the default stderr
...
# Redirect both stdout and stderr to ConsoleLogger
sys.stdout = ConsoleLogger()
sys.stderr = sys.stdout  # Redirect stderr to the same log file as stdout

# Function to add log markers for start and stop of the script
def log_start_stop_marker(marker_type):
    if marker_type == "start":
        print("XXXXX----------------------SCRIPT STARTED----------------------XXXXX")
    elif marker_type == "stop":
        print("XXXXX----------------------SCRIPT STOPPED----------------------XXXXX")
...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/home/kaboom-gaming/my_rfid_project/kaboom-gaming-d326a-0391a77c6216.json"
...
"""PC_IP = "192.168.118.41"  # Replace with the IP of your PC on the same network
PC_PORT = 9999
...
class SocketPrint:
    ...
    def write(self, message):
        # Send the print output through the socket
        if message.strip():  # Don't send empty lines
            send_data(self.client_socket, message)

    def flush(self):
        pass  # Required for compatibility with the print function

# Custom logger handler to send logs through the socket
class SocketLogger(logging.StreamHandler):
    def __init__(self, client_socket):
        super().__init__()
        self.client_socket = client_socket
    ...
    def emit(self, record):
        ...
        send_data(self.client_socket, log_entry)

# Start the client and establish a connection to the PC
client_socket = start_client()

# Redirect standard print to the socket
sys.stdout = SocketPrint(client_socket)

# Setup logging to also send logs through the socket
socket_handler = SocketLogger(client_socket)
socket_handler.setLevel(logging.INFO)
formatter = logging.Formatter('%(asctime)s - %(message)s')
socket_handler.setFormatter(formatter)

# Add the socket handler to the root logger
logging.getLogger().addHandler(socket_handler)

# Example of using print and logging, this will be sent to the PC
...
def username_controller(start_time, end_time, user_name, controllers_value):
    sender_delay = (start_time - datetime.now(IST)).total_seconds()
    global controllers
    def sender_action():
        global controllers
        #global allowed_controllers_updated
        if (start_time < datetime.now(IST) < end_time):
            ...
            allowed_controllers_label.config(text=f"Allowed controllers: {controllers}")
            #allowed_controllers_updated=True
        else:
...
def process_next_booking():
    ...
    if not booking_queue.empty():
        ...
        logging.info(f"Processing next booking from {start_time} to {end_time}")
        total_downtime = timedelta()  # Reset total downtime for the new booking
        schedule_start_stop_timers(start_time, end_time)
        schedule_reboot_after_booking(end_time)
        update_timer()
        print(f"timer updated")
    else:
        current_booking = None
        logging.info("No more bookings in the queue.")
        # No more bookings, we can schedule a reboot if booking_status is False
        if not booking_status:
            logging.info("No active bookings, checking if reboot can be scheduled.")
            check_for_reboot()
...
LAST_REBOOT_FILE = "/home/kaboom-gaming/my_rfid_project/last_reboot.txt"
REBOOT_INTERVAL = timedelta(hours=3)  # 3 hours
reboot_timer = None

def read_last_reboot_time():
    """Read the last reboot time from the persistent file."""
    if os.path.exists(LAST_REBOOT_FILE):
        with open(LAST_REBOOT_FILE, 'r') as file:
            try:
                timestamp_str = file.read().strip()
                last_reboot_time = datetime.strptime(timestamp_str, '%Y-%m-%d %H:%M:%S')

                # Ensure last_reboot_time is timezone-aware
                if last_reboot_time.tzinfo is None:
                    last_reboot_time = IST.localize(last_reboot_time)
...
def write_last_reboot_time():
    """Write the current time as the last reboot time to the file."""
    with open(LAST_REBOOT_FILE, 'w') as file:
        file.write(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))

def reboot_system():
    """Reboot the system and update the last reboot time."""
    logging.info("Rebooting the system now.")
    write_last_reboot_time()
    os.system("sudo reboot")

def schedule_reboot_after_booking(booking_end_time):
    """Schedule a reboot 5 minutes after the current booking ends, or after the next booking if booking_status is True."""
    global reboot_timer
    now = datetime.now(IST)  # Make 'now' timezone-aware by localizing it to IST
    time_until_reboot = (booking_end_time + timedelta(minutes=5)+timedelta(seconds=10)) - now
    last_reboot_time = read_last_reboot_time()

    # Only proceed if the last reboot was more than REBOOT_INTERVAL ago
    if last_reboot_time and (now - last_reboot_time) < REBOOT_INTERVAL:
        logging.info("Reboot skipped: Last reboot was less than 3 hours ago.")
        return

    if booking_status:
        # If booking is active, schedule reboot at the booking's end time
        logging.info("Booking is active. Reboot will be scheduled after the booking ends.")
        time_until_end = (booking_end_time + timedelta(minutes=5)+timedelta(seconds=10)) - now
        if time_until_end.total_seconds() > 0:
            if reboot_timer:
                reboot_timer.cancel()
                logging.info("Previous reboot timer canceled.")
            reboot_timer = threading.Timer(time_until_end.total_seconds(), reboot_system)
            reboot_timer.start()
            logging.info(f"Reboot scheduled for {booking_end_time + timedelta(minutes=5)+timedelta(seconds=10)}")
        else:
            logging.info("Cannot schedule reboot: Time until reboot is negative.")
    else:
        # If no active booking, schedule the reboot as usual
        logging.info("No active booking, scheduling reboot normally.")
        if time_until_reboot.total_seconds() > 0:
            if reboot_timer:
                reboot_timer.cancel()
                logging.info("Previous reboot timer canceled.")
            reboot_timer = threading.Timer(time_until_reboot.total_seconds(), reboot_system)
            reboot_timer.start()
            logging.info(f"Reboot scheduled for {booking_end_time + timedelta(minutes=5)+timedelta(seconds=10)}")
        else:
            logging.info("Cannot schedule reboot: Time until reboot is negative.")

def cancel_reboot_timer():
    """Cancel the currently scheduled reboot, if any."""
    global reboot_timer
    if reboot_timer:
        reboot_timer.cancel()
        logging.info("Reboot timer canceled.")

def handle_booking_updates():
    """Check if a new booking is added and adjust reboot scheduling accordingly."""
    global reboot_timer

    if not current_booking or booking_queue.empty():
        return

    now = datetime.now(IST)
    current_booking_end_time = current_booking[1]  # End time of the current booking
    next_booking = booking_queue.queue[0]  # Peek at the next booking
    next_booking_start_time = next_booking[0]  # Start time of the next booking

    # Check if the new booking start time and current booking end time are less than or equal to 5 minutes apart
    if (next_booking_start_time - current_booking_end_time).total_seconds() <= 300:
        # Cancel the scheduled reboot and delay it until the next booking ends
        logging.info("New booking detected within 5 minutes of the current booking's end. Reboot rescheduled.")
        cancel_reboot_timer()  # Cancel the previously scheduled reboot
        schedule_reboot_after_booking(next_booking[1])  # Reschedule reboot after the new booking ends
    else:
        # Schedule reboot after the current booking ends if no overlapping booking is detected
        schedule_reboot_after_booking(current_booking_end_time)

def check_for_reboot():
    """Check if the system can reboot based on the last reboot time and current conditions."""
    last_reboot_time = read_last_reboot_time()
    print(f"last rebbot time:{last_reboot_time}")
    now = datetime.now(IST)

    # If last reboot was less than 3 hours ago, don't reboot again
    if last_reboot_time and (now - last_reboot_time) < REBOOT_INTERVAL:
        logging.info("Reboot skipped: Last reboot was less than 3 hours ago.")
        return

    # If booking_status is True (active booking), skip reboot
    if booking_status:
        logging.info("Reboot skipped: booking_status is True (active booking).")
        return

    # If there is a scheduled booking within 5 minutes, skip reboot
    if not booking_queue.empty():
        next_booking = booking_queue.queue[0]  # Peek at the next booking in the queue
        next_start_time = next_booking[0]
        if (next_start_time - now).total_seconds() <= 300:  # 5 minutes
            logging.info("Reboot skipped: A booking is scheduled within the next 5 minutes.")
            return

    # If all conditions are satisfied, reboot the system
    logging.info("Conditions satisfied. Rebooting the system now.")
    write_last_reboot_time()
    os.system("sudo reboot")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Log the stop of the script
        log_start_stop_marker("stop")
        sys.stdout.log.close()  # Close the log file properly when the script ends
